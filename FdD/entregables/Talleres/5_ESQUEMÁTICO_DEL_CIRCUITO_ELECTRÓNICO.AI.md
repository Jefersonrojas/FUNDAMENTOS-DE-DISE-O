...
El presente esquema eléctrico es una representación sintetizada de un circuito eléctrico cuya finalidad es controlar la cantidad de vehículos a ingresar en el estacionamiento. Para ello, se inicio insertando el arduino nano en el cual se iban a realizar las conexciones correspondientes. Posterior a ello, se conecto un sensor ultrásonico para que se detecte la presencia del vehículo, también se utilizó dos led donde el del color blanco(que se utilizo en reprentación del led verde) se encendia si el estacionamiento se encontraba vacío, caso contrario a cuando se encendia el led rojo para representar el límite alcanzado. De igual manera, se hizo uso de un servomotor para permitir el ingreso de los vehículos al estacionamiento siempre y cuando este se encuentre vacio. Y finalmente, se insertó una pantalla LCD 16x2 el cual estaba conectado con el potenciómetro en el VO de la pantalla , para imprimir el mensaje de límite alcanzado si el estacionamiento se encontraba lleno.
